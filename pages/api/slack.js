      ...
      if (payload.type === "view_submission" && payload.view.callback_id === "date_range_modal") {
        ...
        try {
          const baseUrl = process.env.NODE_ENV === "development"
            ? "http://localhost:3000"
            : `https://a666-2405-201-e05d-d02f-999f-47ba-47b-561b.ngrok-free.app`;
          ...
          const maxNameLength = Math.max(...allEntries.map(entry => entry.name.length), "Name".length) + 2;
          const maxDateLength = Math.max(...allEntries.map(entry => entry.date.length), "Date".length) + 2;
          const maxPriceLength = Math.max(...allEntries.map(entry => `₹ ${entry.price}/gm`.length), "Price".length) + 2;
          ...
          await app.client.chat.postMessage({
            channel: payload.user.id,
            text: `Price range for ${fromDate} to ${toDate}`, // Added text for accessibility
            blocks: blocks,
          });
        } catch (error) {
